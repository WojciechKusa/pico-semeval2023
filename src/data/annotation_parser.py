...
def get_char_labels(df, picos_mapping, task_id):
    ...
    for counter, row in enumerate(df):
        ...
        if any(word in full_text for word in deleted_list):
            ...
            label_each_char = [0] * len(
                full_text
            )  # Generate a 0 label for each character in the full text
        ...
        if "N.A." not in char_labels:
            ...
            for counter, i in enumerate(tokenized_text):
                ...
                if len(char_to_token_lab) == 1:
                    ...
                    new_text_tokens = [
                        new_text[i:j] for i, j in zip(tok_ind, tok_ind[1:] + [None])
                    ]
                    new_text_labels = [
                        new_labels[i:j] for i, j in zip(tok_ind, tok_ind[1:] + [None])
                    ]
...
if __name__ == "__main__":
    ...
    # open file
    file_path_train = (
        f"../../data/external/st{task_id}_public_data/st{task_id}_train_inc_text.csv"
    )
    file_path_test = (
        f"../../data/external/st{task_id}_public_data/st{task_id}_test_inc_text.csv"
    )
    ...
    labels_train, claim_offsets = get_char_labels(
        train_df, picos_mapping=picos_mapping, task_id=task_id
    )
